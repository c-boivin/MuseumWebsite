...
export function CartSummary() {
  ...
  return (
    /* L'ancre du panier est posée ICI et non sur le panneau lui-même, qui est
       `sticky`. Next écarte les éléments `sticky` et `fixed` de ses cibles de
       défilement : ils réussissent son test « déjà à l'écran » alors que le
       contenu, lui, est ailleurs. La cible était donc ignorée — le lien du
       Header ne descendait pas jusqu'au panier — et, pire, l'ancre restait en
       attente dans le routeur : la navigation SUIVANTE cherchait `#panier`
       dans la page d'arrivée, ne l'y trouvait pas et renonçait à remonter en
       haut. D'où l'arrivée en milieu de page sur /connexion après un passage
       par le panier.

       `self-stretch` : ce conteneur devient la cellule de la grille, qui est
       en `items-start`. Sans lui il se réduirait à la hauteur du panneau, et
       le `sticky` n'aurait plus aucune course à faire.
    ...
    <div id="panier" className="scroll-mt-section-sm self-stretch">
      {/* `sticky` calé sous le header, même calcul que la colonne de filtres :
         le total doit rester visible pendant qu'on parcourt la grille.

         `data-tone="ink"` inverse le contour de focus : sans lui, la
         tabulation jusqu'à « Payer » dessinerait un trait noir sur fond noir. */}
      <aside
        data-tone="ink"
        className={cn(
          "sticky top-[calc(var(--spacing-header)+1.5rem)] rounded-lg bg-ink-deep px-8 py-7 text-paper",
          /* 300 ms : la place est rendue bien avant que le mur atteigne le haut
             de l'écran (0,95 s pour couvrir), et le bloc se retire au lieu de
